...
export default (error, req, res, next) => {
  ...
  if (errorMessage.includes('duplicate key')) {
    logger.log(logger.ERROR, `Responding with a 409 error code and message ${error.message}`);
    return res.sendStatus(409);
  }

  logger.log(logger.ERROR, `Responding with a 500 error code ${JSON.stringify(error)}`);
  next();
